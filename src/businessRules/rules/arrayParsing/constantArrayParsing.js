...
async function searchForPatternsInStringArray(inputData, inputMetaData) {
  ...
  if (inputData && inputData.length > 0) {
    ...
    for (let a = 0; a < inputData.length; a++) {
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.ccurrentMasterStringArrayElementIs + currentMasterStringArrayElement
      );
      if (currentMasterStringArrayElement.includes(bas.cSpace) === false) {
        ...
        for (let b = minStringLength; b <= maxStringLength; b++) {
          ...
          await loggers.consoleLog(
            namespacePrefix + functionName,
            "length of string to compare is: " + JSON.stringify(toString(b))
          );
          ...
          if (currentMasterStringArrayElement.length <= b) {
            await loggers.consoleLog(
              namespacePrefix + functionName,
              "currentMasterStringArrayElement.length is less than b"
            );
            ...
            for (
              ...
              await loggers.consoleLog(
                namespacePrefix + functionName,
                "beginningIndex is: " + beginningIndex
              );
              // console.log('beginningIndex is: ' + beginningIndex);
              let endingIndex = c + b;
              await loggers.consoleLog(
                namespacePrefix + functionName,
                "endingIndex is: " + endingIndex
              );
              ...
              await loggers.consoleLog(
                namespacePrefix + functionName,
                "stringToCompare is: " + stringToCompare
              );
              ...
              for (let d = 0; d < inputData.length; d++) {
                ...
                if (d != a) {
                  ...
                  if (otherStringToCompare.includes(stringToCompare)) {
                    ...
                    if (
                      ruleParsing.processRulesInternal(
                        [
                          ...
                          ruleParsing.getRule(biz.cascertainMatchingElements)
                        ],
                    ...
                    //   console.log("Nikhil haystack src");
                    //   returnData.push(stringToCompare);
        ...
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.cSearchForPatternsInSringArrayMessage2 +
    ...
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cSearchForPatternsInSringArrayMessage4
    );
  }
  await loggers.consoleLog(
    namespacePrefix + functionName,
